...
class MuendlicheNote extends Note {
    ...
    get note() {
        return MathUtil.round((this.experteA * 2 + this.experteB) / 3);
    }

    toString() {
        return (
            super.toString() + ` (66% ${this.experteA} / 33% ${this.experteB})`
        );
    }
}
...
class Student {
    ...
    avg() {
        return (
            this.noten.reduce(
                (previousValue, note) => previousValue + note.note,
                0
            ) / this.noten.length
        );
    }
...
console.log(new MuendlicheNote(deutsch, 3.5, 4));
